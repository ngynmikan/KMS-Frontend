...
export function AccountManagementPage() {
  ...
  const fetchUsers = async () => {
    try {
      setIsLoading(true);
      const [pendingResp, activeResp, inactiveResp] = await Promise.all([
        userService.getPendingUsers(),
        userService.getActiveUsers(),
        userService.getInactiveUsers(),
      ]);

      const pending = extractArray<ManagedUser>(pendingResp);
      const active = extractArray<ManagedUser>(activeResp);
      const inactive = extractArray<ManagedUser>(inactiveResp);

      // Deduplicate users and prioritize status: pending > active > inactive
      const userMap = new Map<number, AppUser>();
      
      pending.forEach(u => userMap.set(u.userId, { ...u, _displayStatus: 'pending' }));
      active.forEach(u => {
        if (!userMap.has(u.userId)) {
          userMap.set(u.userId, { ...u, _displayStatus: 'active' });
        }
      });
      inactive.forEach(u => {
        if (!userMap.has(u.userId)) {
          userMap.set(u.userId, { ...u, _displayStatus: 'inactive' });
        }
      });
