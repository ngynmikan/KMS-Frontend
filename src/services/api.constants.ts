export const API_ENDPOINTS = {
  ...
  // Class Activities
  CLASS_ACTIVITIES: {
    BASE: '/api/ClassActivities',
    BY_ID: (id: number | string) => `/api/ClassActivities/${id}`,
    BY_CLASS: (classId: number | string) => `/api/ClassActivities/class/${classId}`,
    PHOTOS: (activityId: number | string) => `/api/ClassActivities/${activityId}/photos`,
    DELETE_PHOTO: (photoId: number | string) => `/api/ClassActivities/photos/${photoId}`,
  },
  
  // Timetable
  TIMETABLE: {
    BASE: '/api/Timetable',
    BY_ID: (id: number | string) => `/api/Timetable/${id}`,
    BY_CLASS: (classId: number | string) => `/api/Timetable/class/${classId}`,
    BY_TEACHER: (teacherId: number | string) => `/api/Timetable/teacher/${teacherId}`,
  },

  // Evaluations
  EVALUATION: {
    STUDENT_BASE: (studentId: number | string) => `/api/students/${studentId}/evaluations`,
    BY_ID: (studentId: number | string, evaluationId: number | string) => `/api/students/${studentId}/evaluations/${evaluationId}`,
    CRITERIA: '/api/evaluation-criteria',
    CLASS_SUMMARY: (classId: number | string) => `/api/classes/${classId}/evaluations/summary`,
    GOOD_STUDENTS: (classId: number | string) => `/api/classes/${classId}/evaluations/good-students`,
  }
} as const;
